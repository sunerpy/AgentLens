...
use agentlens_core::source::claude_code;
use agentlens_core::source::codex;
use agentlens_core::source::hermes;
...
const CLAUDE_CODE_SOURCE_NAME: &str = "claude-code";
const CODEX_SOURCE_NAME: &str = "codex";
const HERMES_SOURCE_NAME: &str = "hermes";
...
const USAGE: &str = "用法: agentlens-collector collect --since <cursor_ms> \
     [--source <opencode|claude-code|codex|hermes>] [--data-dir <path>] \
     [--request-base64url <payload>] [--snapshot]";
...
/// Adapter one `collect` invocation runs.
///
/// A run selects exactly one source so its `--since` cursor unambiguously belongs to that source's
/// `source_cursor` row. Reporting two sources under one cursor would advance a watermark past
/// records the other source never scanned.
...
impl CollectorSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::OpenCode => SOURCE_NAME,
            Self::ClaudeCode => CLAUDE_CODE_SOURCE_NAME,
            Self::Codex => CODEX_SOURCE_NAME,
            Self::Hermes => HERMES_SOURCE_NAME,
        }
    }

    fn parse(value: &str) -> CollectorResult<Self> {
        match value {
            SOURCE_NAME => Ok(Self::OpenCode),
            CLAUDE_CODE_SOURCE_NAME => Ok(Self::ClaudeCode),
            CODEX_SOURCE_NAME => Ok(Self::Codex),
            HERMES_SOURCE_NAME => Ok(Self::Hermes),
            other => Err(CollectorError::InvalidInput(format!(
                "未知 source {other:?}；支持 {SOURCE_NAME}、{CLAUDE_CODE_SOURCE_NAME}、{CODEX_SOURCE_NAME} 与 {HERMES_SOURCE_NAME}"
            ))),
        }
    }
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NoDataDirectory(message) => write!(
                formatter,
                "未找到可用的数据目录：{message}；请用 --data-dir 指向所选采集源的数据目录"
            ),
            ...
            Self::SourceUnreadable(message) => write!(formatter, "采集源不可读：{message}"),
...
fn parse_collect_request(args: &[OsString]) -> CollectorResult<CollectRequest> {
    ...
    while index < args.len() {
        ...
        match flag {
            ...
            "--source" => {
                ensure_not_set(source.is_some(), "--source")?;
                let value = required_value(args, index, "--source")?;
                let text = value.to_str().ok_or_else(|| {
                    CollectorError::InvalidInput("--source 必须是有效的 UTF-8".into())
                })?;
                source = Some(CollectorSource::parse(text)?);
                index += 2;
            }
            "--request-base64url" => {
    ...
    if let Some(encoded) = payload {
        ...
        if let Some(payload_source) = decoded.source {
            source = Some(CollectorSource::parse(&payload_source)?);
        }
    }

    let source = source.unwrap_or_default();
    if snapshot && source != CollectorSource::OpenCode {
        // `VACUUM INTO` has no analogue for append-only JSONL transcripts. Silently ignoring the
        // flag would let a caller believe it got a point-in-time copy it never received.
        return Err(CollectorError::InvalidInput(format!(
            "--snapshot 仅适用于 {SOURCE_NAME}；{} 的 transcript 是追加写的 JSONL，没有等价的一致性快照",
            source.as_str()
        )));
    }
...
fn collect_with_space_probe(
    ...
    let scan = match request.source {
        CollectorSource::OpenCode => scan_opencode(request, identity, snapshot_dir, space_probe)?,
        CollectorSource::ClaudeCode => scan_claude_code(request, identity)?,
        CollectorSource::Codex => scan_codex(request, identity)?,
        CollectorSource::Hermes => scan_hermes(request, identity)?,
    };

    validate_scan_completion(request, &scan.records)?;
...
fn scan_opencode(
    ...
    ensure_reached_eof(scan_result.reached_eof, &scan_result.skip_reason)?;
    ...
    Ok(SourceScan {
        ...
        data_dir: database
            ...
            .to_string_lossy()
            .into_owned(),
        cutoff: source_cutoff(scan_result.observed_max_time_updated, request.since),
...
fn scan_claude_code(
    ...
    let projects_dir = resolve_projects_dir(request.data_dir.as_deref())?;
    let mut records = Vec::new();
    let scan_request = claude_code::ScanRequest::live(identity.host_id(), Some(request.since));
    let scan_result = claude_code::scan_projects_dir(&projects_dir, &scan_request, |batch| {
        records.extend_from_slice(batch);
        Ok(())
    })
    .map_err(map_claude_code_error)?;

    ensure_reached_eof(scan_result.reached_eof, &scan_result.skip_reason)?;
    if scan_result.diagnostics.files_unreadable > 0 {
        // An unreadable transcript is a hole in this window that a later scan cannot revisit once
        // the 30-day cleanup removes the file, so the round must not be reported as complete.
        return Err(CollectorError::SourceUnreadable(format!(
            "{} 个 transcript 文件不可读，本轮覆盖不完整",
            scan_result.diagnostics.files_unreadable
        )));
    }

    Ok(SourceScan {
        records,
        data_dir: projects_dir.to_string_lossy().into_owned(),
        cutoff: source_cutoff(scan_result.observed_max_time_updated, request.since),
        eligible_count: scan_result.eligible_count,
        skipped_count: scan_result.skipped_count,
    })
...
fn scan_codex(request: &CollectRequest, identity: &MachineIdentity) -> CollectorResult<SourceScan> {
    let data_dir = resolve_codex_data_dir(request.data_dir.as_deref())?;
    let mut records = Vec::new();
    let scan_request = codex::ScanRequest::live(identity.host_id(), Some(request.since));
    let scan_result = codex::scan_data_dir(&data_dir, &scan_request, |batch| {
        records.extend_from_slice(batch);
        Ok(())
    })
...
fn scan_hermes(
    ...
    let data_dir = resolve_hermes_data_dir(request.data_dir.as_deref())?;
    let mut records = Vec::new();
    let scan_request = hermes::ScanRequest::live(identity.host_id(), Some(request.since));
    let scan_result = hermes::scan_data_dir(&data_dir, &scan_request, |batch| {
        records.extend_from_slice(batch);
        Ok(())
    })
    .map_err(map_hermes_error)?;
...
/// Clamps a source cutoff so a cursor can never move backwards across rounds.
const fn source_cutoff(observed_max: Option<i64>, since: i64) -> i64 {
    match observed_max {
        Some(value) if value > since => value,
        _ => since,
    }
}

fn ensure_reached_eof<R: fmt::Debug>(
    reached_eof: bool,
    skip_reason: &Option<R>,
) -> CollectorResult<()> {
    if reached_eof {
        return Ok(());
    }
    Err(CollectorError::SourceUnreadable(format!(
        "扫描未到达 EOF：{skip_reason:?}"
    )))
}
...
fn validate_scan_completion(
    ...
    let expected = request.source.as_str();
    if records.iter().any(|record| record.source != expected) {
...
fn resolve_projects_dir(data_dir: Option<&Path>) -> CollectorResult<PathBuf> {
    let Some(directory) = data_dir else {
        return claude_code::discover_projects_dir().map_err(map_claude_code_error);
    };
    if !directory.is_dir() {
    ...
    // `--data-dir` may name either the Claude Code config directory or the `projects` tree itself.
    let nested = directory.join(claude_code::PROJECTS_DIRECTORY);
    if nested.is_dir() {
        return Ok(nested);
    }
    if directory.file_name() == Some(OsStr::new(claude_code::PROJECTS_DIRECTORY)) {
        return Ok(directory.to_path_buf());
    }
    Err(CollectorError::NoDataDirectory(format!(
        "{} 中不存在 {} 子目录",
        directory.display(),
        claude_code::PROJECTS_DIRECTORY
    )))
}

fn map_claude_code_error(error: claude_code::ClaudeCodeError) -> CollectorError {
    if matches!(
        &error,
...
fn resolve_codex_data_dir(data_dir: Option<&Path>) -> CollectorResult<PathBuf> {
    let Some(directory) = data_dir else {
        return codex::discover_data_dir().map_err(map_codex_error);
    };
    if !directory.is_dir() {
        return Err(CollectorError::NoDataDirectory(format!(
            "{} 不是目录",
    ...
    if directory.join(codex::SESSIONS_DIRECTORY).is_dir()
        || directory.join(codex::ARCHIVED_SESSIONS_DIRECTORY).is_dir()
    {
        return Ok(directory.to_path_buf());
    }
    Err(CollectorError::NoDataDirectory(format!(
...
fn map_codex_error(error: codex::CodexError) -> CollectorError {
    if matches!(&error, codex::CodexError::DataDirectoryNotFound { .. }) {
        CollectorError::NoDataDirectory(error.to_string())
    } else {
        CollectorError::SourceUnreadable(error.to_string())
    }
...
fn resolve_hermes_data_dir(data_dir: Option<&Path>) -> CollectorResult<PathBuf> {
    let Some(directory) = data_dir else {
        return hermes::discover_data_dir().map_err(map_hermes_error);
    };
    if !directory.is_dir() {
        return Err(CollectorError::NoDataDirectory(format!(
            "{} 不是目录",
    ...
    if directory.join(hermes::STATE_DATABASE).is_file() {
        return Ok(directory.to_path_buf());
    }
    Err(CollectorError::NoDataDirectory(format!(
        "{} 中不存在 {}",
        directory.display(),
        hermes::STATE_DATABASE
    )))
}

fn map_hermes_error(error: hermes::HermesError) -> CollectorError {
    if matches!(&error, hermes::HermesError::DataDirectoryNotFound { .. }) {
        CollectorError::NoDataDirectory(error.to_string())
    } else {
        CollectorError::SourceUnreadable(error.to_string())
    }
...
mod tests {
    ...
    fn request(data_dir: &Path, snapshot: bool) -> CollectRequest {
        CollectRequest {
            ...
            snapshot,
            source: CollectorSource::OpenCode,
        }
    }

    fn claude_request(data_dir: &Path) -> CollectRequest {
        CollectRequest {
            since: 0,
            data_dir: Some(data_dir.to_path_buf()),
            snapshot: false,
            source: CollectorSource::ClaudeCode,
        }
    }
    ...
    fn synthetic_claude_root(root: &Path) -> claude_code::fixture::Manifest {
    ...
    #[test]
    fn collector_claude_code_source_emits_v1_ndjson_with_its_own_meta_and_cutoff() {
        let temp = tempfile::tempdir().expect("claude tempdir");
        let manifest = synthetic_claude_root(temp.path());
        let snapshot_dir = tempfile::tempdir().expect("snapshot tempdir");

        let collection = collect_with_space_probe(
            &claude_request(temp.path()),
            &identity(),
            "fixture-host",
            snapshot_dir.path(),
            &abundant_space,
        )
        .expect("collect claude-code source");

        assert_eq!(collection.meta.protocol_version, PROTOCOL_VERSION);
        assert_eq!(collection.meta.sources.len(), 1);
        let source_meta = &collection.meta.sources[0];
        assert_eq!(source_meta.source, CLAUDE_CODE_SOURCE_NAME);
        assert_eq!(source_meta.eligible_count, manifest.eligible);
        assert_eq!(source_meta.skipped_count, manifest.skipped);
        assert_eq!(source_meta.scan_window.since, 0);
        assert_eq!(source_meta.scan_window.cutoff, manifest.max_timestamp_ms);
        assert_eq!(
            source_meta.data_dir,
            temp.path()
                .join(claude_code::PROJECTS_DIRECTORY)
                .to_string_lossy()
        );
        assert_eq!(collection.records.len() as u64, manifest.eligible);
        assert!(collection
            .records
            .iter()
            .all(|record| record.source == CLAUDE_CODE_SOURCE_NAME
                && record.cost.is_none()
                && record.host_id == identity().host_id()));

        let mut ndjson = Vec::new();
        write_ndjson(&mut ndjson, &collection).expect("write ndjson");
        let text = String::from_utf8(ndjson).expect("utf-8 ndjson");
        let mut lines = text.lines();
        let meta: CollectorMetaV1 =
            serde_json::from_str(lines.next().expect("meta line")).expect("decode meta");
        assert_eq!(meta, collection.meta);
        assert_eq!(lines.count() as u64, manifest.eligible);

        println!(
            "collector_claude source={} eligible={} skipped={} cutoff={} records={}",
            source_meta.source,
            source_meta.eligible_count,
            source_meta.skipped_count,
            source_meta.scan_window.cutoff,
            collection.records.len()
        );
    }

    #[test]
    fn collector_codex_source_emits_v1_ndjson_and_validates_data_roots() {
        ...
        let request = CollectRequest {
            since: 0,
            data_dir: Some(codex_root.clone()),
            snapshot: false,
            source: CollectorSource::Codex,
        };
        let collection =
            collect_with_space_probe(&request, &identity(), "fixture-host", temp.path(), &|_| {
                Ok(u64::MAX)
            })
            .expect("collect Codex source");
        assert_eq!(collection.records.len(), 1);
        assert_eq!(collection.records[0].source, CODEX_SOURCE_NAME);
        assert_eq!(collection.records[0].provider_id, "openai");
        assert_eq!(collection.records[0].model_id, "gpt-5.4");
        let source_meta = &collection.meta.sources[0];
        assert_eq!(source_meta.source, CODEX_SOURCE_NAME);
        assert_eq!(source_meta.eligible_count, 1);
        assert_eq!(source_meta.skipped_count, 2);
        assert_eq!(source_meta.scan_window.cutoff, 1_784_200_093_000);

        let mut ndjson = Vec::new();
        write_ndjson(&mut ndjson, &collection).expect("write Codex ndjson");
        assert_eq!(
        ...
        assert_eq!(
            resolve_codex_data_dir(Some(&codex_root)).expect("resolve sessions root"),
            codex_root
        );

        let archived_root = temp.path().join("archived-only");
        fs::create_dir_all(archived_root.join(codex::ARCHIVED_SESSIONS_DIRECTORY))
            .expect("create archived-only root");
        assert_eq!(
            resolve_codex_data_dir(Some(&archived_root)).expect("resolve archived root"),
            archived_root
        );
        assert!(matches!(
            resolve_codex_data_dir(Some(temp.path())),
            Err(CollectorError::NoDataDirectory(_))
        ));
        assert!(matches!(
            resolve_codex_data_dir(Some(&rollout)),
            Err(CollectorError::NoDataDirectory(_))
        ));
        assert!(matches!(
            map_codex_error(codex::CodexError::DataDirectoryNotFound {
                probed_paths: vec![]
            }),
            CollectorError::NoDataDirectory(_)
        ));
        assert!(matches!(
            map_codex_error(codex::CodexError::InvalidBatchSize),
            CollectorError::SourceUnreadable(_)
        ));
    }

    #[test]
    fn collector_hermes_source_emits_session_v1_ndjson_and_validates_data_roots() {
        let temp = tempfile::tempdir().expect("tempdir");
        let database = temp.path().join(hermes::STATE_DATABASE);
        let connection = Connection::open(&database).expect("create Hermes state database");
        connection
            .execute_batch(
        ...
        connection
            .execute(
                ...
                params!["session-hermes", 1_784_200_093.75_f64],
            )
        ...
        let request = CollectRequest {
            ...
            snapshot: false,
            source: CollectorSource::Hermes,
        };
        let collection =
            collect_with_space_probe(&request, &identity(), "fixture-host", temp.path(), &|_| {
                Ok(u64::MAX)
            })
            .expect("collect Hermes source");

        assert_eq!(collection.records.len(), 1);
        let record = &collection.records[0];
        assert_eq!(record.source, HERMES_SOURCE_NAME);
        assert_eq!(
            record.granularity,
            agentlens_core::archive::UsageGranularity::Session
        );
        assert_eq!(record.message_id, "session-hermes");
        assert_eq!(record.session_id, "session-hermes");
        assert_eq!(record.provider_id, "anthropic");
        assert_eq!(record.model_id, "claude-sonnet-4-5");
        assert_eq!(record.tok_input, 10);
        assert_eq!(record.tok_output, 20);
        assert_eq!(record.tok_cache_read, 30);
        assert_eq!(record.tok_cache_write, 40);
        assert_eq!(record.tok_reasoning, 50);
        assert!(!record.is_incomplete);

        let source_meta = &collection.meta.sources[0];
        assert_eq!(source_meta.source, HERMES_SOURCE_NAME);
        assert_eq!(source_meta.data_dir, temp.path().to_string_lossy());
        assert_eq!(source_meta.eligible_count, 1);
        assert_eq!(source_meta.skipped_count, 0);
        assert_eq!(source_meta.scan_window.since, 0);
        assert_eq!(source_meta.scan_window.cutoff, 1_784_200_093_750);

        let mut ndjson = Vec::new();
        write_ndjson(&mut ndjson, &collection).expect("write Hermes ndjson");
        assert_eq!(
            String::from_utf8(ndjson)
                .expect("utf-8 Hermes ndjson")
                .lines()
                .count(),
            2
        );
        assert_eq!(
            resolve_hermes_data_dir(Some(temp.path())).expect("resolve Hermes root"),
            temp.path()
        );
        assert!(matches!(
            resolve_hermes_data_dir(Some(&database)),
            Err(CollectorError::NoDataDirectory(_))
        ));
        let empty = tempfile::tempdir().expect("empty tempdir");
        assert!(matches!(
            resolve_hermes_data_dir(Some(empty.path())),
            Err(CollectorError::NoDataDirectory(_))
        ));
        assert!(matches!(
            map_hermes_error(hermes::HermesError::DataDirectoryNotFound {
                probed_paths: vec![]
            }),
            CollectorError::NoDataDirectory(_)
        ));
        assert!(matches!(
            map_hermes_error(hermes::HermesError::InvalidBatchSize),
            CollectorError::SourceUnreadable(_)
        ));
    }

    #[test]
    fn collector_source_selector_parses_all_adapters_and_rejects_unsupported_combinations() {
        let opencode = parse_collect_request(&arguments(&[
            "collect", "--since", "5", "--source", "opencode",
        ]))
        .expect("explicit opencode source");
        assert_eq!(opencode.source, CollectorSource::OpenCode);

        let claude = parse_collect_request(&arguments(&[
            "collect",
            "--since",
            "5",
            "--source",
        ...
        .expect("explicit claude-code source");
        assert_eq!(claude.source, CollectorSource::ClaudeCode);
        assert_eq!(claude.source.as_str(), CLAUDE_CODE_SOURCE_NAME);

        let codex = parse_collect_request(&arguments(&[
            "collect", "--since", "5", "--source", "codex",
        ]))
        .expect("explicit codex source");
        assert_eq!(codex.source, CollectorSource::Codex);
        assert_eq!(codex.source.as_str(), CODEX_SOURCE_NAME);

        let hermes = parse_collect_request(&arguments(&[
            "collect", "--since", "5", "--source", "hermes",
        ]))
        .expect("explicit Hermes source");
        assert_eq!(hermes.source, CollectorSource::Hermes);
        assert_eq!(hermes.source.as_str(), HERMES_SOURCE_NAME);

        let default = parse_collect_request(&arguments(&["collect", "--since", "5"]))
            .expect("omitted source defaults to opencode");
        assert_eq!(
            default.source,
            CollectorSource::OpenCode,
            "the default must stay opencode so existing callers are unaffected"
        );
        assert_eq!(CollectorSource::default(), CollectorSource::OpenCode);

        for values in [
            vec!["collect", "--since", "5", "--source", "gemini"],
            vec!["collect", "--since", "5", "--source"],
            vec![
                "collect", "--since", "5", "--source", "opencode", "--source", "opencode",
            ],
        ] {
            let error = parse_collect_request(&arguments(&values))
                .expect_err("invalid --source usage must fail");
            assert_eq!(error.exit_code(), 1);
        }

        let snapshot_conflict = parse_collect_request(&arguments(&[
            "collect",
            "--since",
            "5",
            "--source",
        ...
        .expect_err("--snapshot has no analogue for append-only transcripts");
        assert_eq!(snapshot_conflict.exit_code(), 1);
        assert!(snapshot_conflict.to_string().contains("--snapshot"));

        let payload = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .encode(br#"{"since":9,"source":"claude-code"}"#);
        let encoded = parse_collect_request(&arguments(&[
            "collect",
            "--since",
            "5",
            "--source",
        ...
        .expect("payload overrides the flag");
        assert_eq!(encoded.source, CollectorSource::ClaudeCode);
        assert_eq!(encoded.since, 9);

        let bad_payload = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .encode(br#"{"since":9,"source":"nope"}"#);
        let error = parse_collect_request(&arguments(&[
            "collect",
            "--since",
            "5",
            "--request-base64url",
        ...
        .expect_err("an unknown payload source must fail");
        assert_eq!(error.exit_code(), 1);
    }

    #[test]
    fn collector_claude_data_dir_accepts_either_root_and_reports_a_missing_tree() {
        let temp = tempfile::tempdir().expect("claude tempdir");
        let _manifest = synthetic_claude_root(temp.path());
        let projects = temp.path().join(claude_code::PROJECTS_DIRECTORY);

        assert_eq!(
            resolve_projects_dir(Some(temp.path())).expect("config root resolves"),
            projects
        );
        assert_eq!(
            resolve_projects_dir(Some(&projects)).expect("projects root resolves"),
            projects
        );

        let empty = temp.path().join("empty");
        fs::create_dir_all(&empty).expect("create empty dir");
        let missing = resolve_projects_dir(Some(&empty))
            .expect_err("a directory without projects/ must be rejected");
        assert_eq!(missing.exit_code(), 2);
        assert!(missing.to_string().contains("projects"));

        let not_a_dir = temp.path().join("file.txt");
        fs::write(&not_a_dir, b"x").expect("write file");
        let wrong_kind =
            resolve_projects_dir(Some(&not_a_dir)).expect_err("a file is not a data dir");
        assert_eq!(wrong_kind.exit_code(), 2);

        let unreadable = map_claude_code_error(claude_code::ClaudeCodeError::InvalidBatchSize);
        assert_eq!(unreadable.exit_code(), 4);
        let absent = map_claude_code_error(claude_code::ClaudeCodeError::ProjectsNotFound {
            probed_paths: vec![empty.clone()],
        });
        assert_eq!(absent.exit_code(), 2);

        let collect_error = collect_with_space_probe(
            &claude_request(&empty),
            &identity(),
            "fixture-host",
            temp.path(),
            &abundant_space,
        )
        .expect_err("an absent transcript tree must fail the round");
        assert_eq!(collect_error.exit_code(), 2);
    }

    #[test]
    fn collector_scan_completion_rejects_interruption_partial_scan_and_wrong_source() {
        use agentlens_core::source::opencode::ScanSkipReason;
        ...
        SNAPSHOT_INTERRUPTED.store(true, Ordering::SeqCst);
        let interrupted = validate_scan_completion(&snapshot_request, &[])
            .expect_err("interrupted snapshot must not publish partial data");
        SNAPSHOT_INTERRUPTED.store(false, Ordering::SeqCst);
        assert!(matches!(interrupted, CollectorError::Interrupted));

        assert!(ensure_reached_eof(true, &None::<ScanSkipReason>).is_ok());
        let partial_error = ensure_reached_eof(false, &Some(ScanSkipReason::Busy))
        ...
        let claude_partial = ensure_reached_eof(
            false,
            &Some(
                agentlens_core::source::claude_code::ScanSkipReason::Interrupted(
                    "sink stop".into(),
                ),
            ),
        )
        .expect_err("an interrupted Claude Code scan must fail the same way");
        assert_eq!(claude_partial.exit_code(), 4);

        assert_eq!(source_cutoff(None, 5), 5);
        assert_eq!(source_cutoff(Some(3), 5), 5);
        assert_eq!(source_cutoff(Some(9), 5), 9);
        ...
        let source_error = validate_scan_completion(&request(&directory, false), &[wrong_source])
        ...
        let claude_record = collection.records[0].clone();
        let claude_error = validate_scan_completion(&claude_request(&directory), &[claude_record])
            .expect_err("an OpenCode record must not pass a Claude Code round");
        assert_eq!(claude_error.exit_code(), 4);
    }
