...
const BREAKDOWN_COST_SQL: &str = "SELECT
        source, agent_key, provider_id, model_id, variant,
        tok_input, tok_output, tok_cache_read, tok_cache_write,
        cost, cost_source
    FROM usage_record
    WHERE is_incomplete = 0
...
/// Queries a coverage-aware time series; uncovered buckets carry null values, never synthetic zero.
...
pub fn query_breakdown(
    ...
    while let Some(row) = rows.next()? {
        ...
        let raw = RawAggregate {
            ...
            active_session_count: row.get(12)?,
        };
        groups.insert(
            key.clone(),
            BreakdownRow {
...
fn add_breakdown_costs(
    ...
    while let Some(row) = rows.next()? {
        ...
        let source_text: String = row.get(10)?;
        if let Some(group) = groups.get_mut(&key) {
            group.cost.add(prices.resolve_cost(
                &provider_id,
                &model_id,
                TokenCounts {
                    input: nonnegative("tok_input", row.get(5)?)?,
                    output: nonnegative("tok_output", row.get(6)?)?,
                    cache_read: nonnegative("tok_cache_read", row.get(7)?)?,
                    cache_write: nonnegative("tok_cache_write", row.get(8)?)?,
                },
                row.get(9)?,
                parse_cost_source(&source_text)?,
            ));
