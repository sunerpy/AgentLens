...
//! 同一适配器的 live / bak / legacy 三种来源共用同一个 `source` 键（`source` 由
//! [`IngestRound::begin_for_source`] 绑定，`opencode` 与 `claude-code` 各占一份），来源层级另存
...
/// Conditional upsert used for every normalized record.
///
...
///
/// `source` is bound rather than inlined so one archive can hold several adapters. The unique key
/// is `(host_id, source, message_id)`, so two adapters that happen to mint the same message id
/// still occupy separate rows.
pub const USAGE_UPSERT_SQL: &str = "INSERT INTO usage_record (
    ...
    cost, cost_source, is_incomplete, project_dir, granularity
) VALUES (
    ?1, ?24, ?2, ?3,
    ?4, ?5, ?6,
    ?7, ?8, ?9, ?10,
...
pub enum IngestError {
    ...
    EmptyHostId,
    /// The round source is blank and cannot form a stable deduplication key.
    #[error("archive ingest source is empty")]
    EmptySource,
    ...
    /// Every record in a round must carry the round's source key so the unique key can deduplicate.
    #[error("archive ingest source {found:?} is invalid; expected {expected:?}")]
    InvalidSource {
        /// Source key fixed when the round began.
        expected: String,
...
impl<'connection> IngestRound<'connection> {
    ...
    pub fn begin(
        ...
        Self::begin_for_source(connection, host_id, OPENCODE_SOURCE, origin)
    }

    /// Begins one all-or-nothing round for an explicit archive source key.
    ///
    /// Each source keeps its own `source_cursor` row, so adapters advance independently and one
    /// interrupted adapter never rewinds another.
    pub fn begin_for_source(
        connection: &'connection mut Connection,
        host_id: impl Into<String>,
    ...
    /// Returns the archive source key every record in this round must carry.
    ...
    pub fn ingest_batch(&mut self, records: &[NormalizedUsageRecord]) -> Result<()> {
        ...
        for record in records {
            let values = ValidatedRecord::new(record, &self.host_id, &self.source, self.origin)?;
    ...
    ///
    /// `reached_eof` is the only signal permitted to advance a watermark: a partial round rolls
    /// back every delivered batch so a later scan re-reads the same window instead of trusting a
    ...
    fn commit_round(
        ...
        if self.origin == Origin::Live {
            if let Some(observed_max) = observed_max_time_updated {
                transaction.execute(
                    CURSOR_UPSERT_SQL,
                    params![self.host_id, observed_max, self.source],
                )?;
...
///
/// Ascending priority is a durability requirement, not a preference. Only a [`Origin::Live`] round
/// writes `source_cursor`, so committing live first and then failing on a backfill tier would leave
/// the watermark past records that never landed — the next scan filters them out of its window and
/// they are lost for good. Committing the backfill tiers first means a crash before the live commit
/// leaves the watermark untouched, and the replay re-upserts the same rows idempotently.
const ORIGIN_COMMIT_ORDER: [Origin; 3] = [Origin::Legacy, Origin::Bak, Origin::Live];

/// Ingests a materialized record set as one round per provenance tier it contains.
///
/// [`IngestRound`] binds exactly one [`Origin`], because that origin is what decides whether the
/// round may advance the watermark. Every adapter but Codex stamps one origin on every record, so a
/// single round is enough for them. Codex derives the origin from the file tree — `sessions/` is
/// live and an unshadowed `archived_sessions/` copy is backup — so one round rejects the first
/// archived record with [`IngestError::OriginMismatch`] and rolls back the live records too.
///
/// The live tier always runs, even with no live records, so an EOF-complete empty scan still
/// reports `committed`.
///
/// `live_cursor` overrides the watermark the live round commits, for transports that report a cutoff
/// instead of deriving one from records; `None` derives it from the live tier's own
/// `max(source_time_updated)`. A backfill tier never contributes to the watermark either way.
pub fn ingest_by_origin(
    ...
    for origin in ORIGIN_COMMIT_ORDER {
        let tier: Vec<NormalizedUsageRecord> = records
            .iter()
            .filter(|record| record.origin == origin)
            .cloned()
            .collect();
        if tier.is_empty() && origin != Origin::Live {
            continue;
        }
        let cursor = if origin == Origin::Live {
            live_cursor.or_else(|| tier.iter().map(|record| record.source_time_updated).max())
        } else {
            None
        };
        ...
        if origin == Origin::Live {
            merged.cursor_time_updated = stats.cursor_time_updated;
        }
    }
    merged.committed = committed_every_tier;
    Ok(merged)
}
...
impl ValidatedRecord {
    fn new(
        ...
        if record.source != round_source {
            return Err(IngestError::InvalidSource {
                expected: round_source.to_owned(),
...
mod tests {
    ...
    fn origin_tiers(connection: &Connection) -> Vec<(String, i64)> {
        let mut statement = connection
            .prepare(
                "SELECT origin, count(*) FROM usage_record
                 GROUP BY origin ORDER BY origin_priority",
            )
    ...
    /// A single round binds one origin, so a Codex scan carrying both `sessions/` (live) and
    /// `archived_sessions/` (backup) records used to abort on the first backup record and roll the
    /// live ones back with it. Every tier must land, and only the live tier may move the watermark.
    #[test]
    fn ingest_by_origin_commits_every_tier_and_takes_the_cursor_from_live_only() {
        let temp = tempfile::tempdir().expect("archive tempdir");
        let mut archive = Archive::open_in_data_dir(temp.path()).expect("open archive");
        let mixed = vec![
            record("tiered-live", Origin::Live, 300, 5),
            record("tiered-bak", Origin::Bak, 900, 6),
            record("tiered-legacy", Origin::Legacy, 950, 7),
        ];
        ...
        .expect("a mixed-origin payload must ingest");

        assert!(stats.committed);
        assert_eq!(stats.received_records, 3);
        assert_eq!(stats.changed_records, 3);
        assert_eq!(query_count(archive.connection()), 3);
        assert_eq!(
            origin_tiers(archive.connection()),
            vec![
                ("legacy".to_owned(), 1),
                ("bak".to_owned(), 1),
                ("live".to_owned(), 1)
        ...
        // 900 and 950 are the newer backfill timestamps; picking either would let a backfill skip
        // live data the next incremental window would then never read.
        assert_eq!(stats.cursor_time_updated, Some(300));
        assert_eq!(
    ...
    #[test]
    fn ingest_by_origin_honours_an_explicit_live_cursor_and_an_empty_payload() {
        let temp = tempfile::tempdir().expect("archive tempdir");
        let mut archive = Archive::open_in_data_dir(temp.path()).expect("open archive");

        let empty = ingest_by_origin(
            archive.connection_mut(),
            TEST_HOST,
            OPENCODE_SOURCE,
            &[],
        ...
        .expect("an empty EOF-complete payload must still commit");
        assert!(empty.committed);
        assert_eq!(empty.received_records, 0);
        assert_eq!(empty.cursor_time_updated, None);

        let overridden = ingest_by_origin(
            archive.connection_mut(),
            TEST_HOST,
            OPENCODE_SOURCE,
            &[record("tiered-remote", Origin::Bak, 700, 9)],
            true,
            Some(4_242),
        )
        .expect("a transport cutoff must override the derived cursor");
        assert_eq!(overridden.cursor_time_updated, Some(4_242));
        assert_eq!(
            read_cursor(archive.connection(), TEST_HOST).expect("read cursor"),
            Some(4_242),
            "a remote round reports its own cutoff even when it delivered only backup rows"
        );
    }

    #[test]
    fn ingest_by_origin_rolls_back_every_tier_when_the_scan_never_reached_eof() {
        let temp = tempfile::tempdir().expect("archive tempdir");
        let mut archive = Archive::open_in_data_dir(temp.path()).expect("open archive");
        ...
        let stats = ingest_by_origin(
            ...
            OPENCODE_SOURCE,
            &[
                record("partial-live", Origin::Live, 310, 5),
                record("partial-bak", Origin::Bak, 210, 6),
            ],
            false,
            None,
        )
        .expect("an interrupted payload reports rather than errors");

        assert!(!stats.committed);
        assert_eq!(
            query_count(archive.connection()),
            0,
            "an interrupted round must not leave a partial tier behind"
        );
        assert_eq!(
            read_cursor(archive.connection(), TEST_HOST).expect("read cursor"),
            None
    ...
    fn ingest_round_rejects_identity_and_provenance_mismatches() {
        ...
        let cases = [
            ...
            (
                ...
                "source \"codex\" is invalid; expected \"opencode\"",
            ),
            (
