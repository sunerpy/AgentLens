...
mod tests {
    ...
    fn discovery_scan_default_sink_interruption_and_overlap_boundaries_are_observable() {
        ...
        let missing_explicit = home.path().join("missing-explicit");
        let missing_home = home.path().join("missing-home");
        let error = discover_data_dir_from(Some(&missing_explicit), Some(&missing_home))
            .expect_err("missing discovery candidates must fail");
        let text = error.to_string();
        assert!(text.contains(&missing_explicit.display().to_string()));
        assert!(text.contains(&missing_home.join(".hermes").display().to_string()));
