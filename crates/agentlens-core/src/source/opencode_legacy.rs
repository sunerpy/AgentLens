...
use crate::query::{
    AggregateFilters, CoverageLookup, CoverageShortfall, CoverageStatus, TimeBucket,
};
...
impl CoverageStore {
    ...
    /// Pairs the aggregate filters select. Shared by `status` and `shortfalls` so the set a
    /// bucket's tri-state is computed over can never diverge from the set it is explained by.
    fn selected_pairs<'a>(
        &'a self,
        filters: &'a AggregateFilters,
    ) -> impl Iterator<Item = (&'a (String, String), &'a Vec<CoverageInterval>)> {
        self.pairs.iter().filter(move |((host_id, source), _)| {
            filters
                .host_id
                .as_deref()
...
impl CoverageLookup for CoverageStore {
    fn status(&self, bucket: &TimeBucket, filters: &AggregateFilters) -> CoverageStatus {
        ...
        let selected = self.selected_pairs(filters);
    ...
    fn shortfalls(
        ...
        self.selected_pairs(filters)
            .filter(|(_, intervals)| !intervals.iter().any(|interval| interval.covers(bucket)))
            .map(|((host_id, source), intervals)| CoverageShortfall {
                host_id: host_id.clone(),
                source: source.clone(),
                partial: intervals.iter().any(|interval| interval.intersects(bucket)),
            })
            .collect()
    }
}
...
mod tests {
    ...
    /// A `Partial` bucket must name the pairs behind it, and distinguish the two causes: a pair
    /// that archived none of the bucket (`partial: false`) reads to the user as "that source was
    /// never collected here", while a pair that archived only part of it (`partial: true`) reads
    /// as "collection started mid-bucket". A pair that fully covers the bucket is never listed.
    #[test]
    fn legacy_partial_bucket_names_missing_and_half_covering_pairs_but_not_full_ones() {
        let temp = tempfile::tempdir().expect("coverage tempdir");
        let mut archive = Archive::open_in_data_dir(temp.path()).expect("open archive");
        for (host_id, interval) in [
            ("host-covered", CoverageInterval::new(0, 100)),
            ("host-half", CoverageInterval::new(0, 50)),
            ("host-elsewhere", CoverageInterval::new(200, 300)),
        ] {
            replace_origin_coverage(
                archive.connection_mut(),
                host_id,
                OPENCODE_SOURCE,
                Origin::Live,
                Some(interval.expect("valid interval")),
            )
            .expect("write coverage row");
        }
        let coverage = CoverageStore::load(archive.connection()).expect("load coverage snapshot");
        let target = bucket(0, 100);
        let filters = AggregateFilters::default();

        assert_eq!(coverage.status(&target, &filters), CoverageStatus::Partial);
        assert_eq!(
            coverage.shortfalls(&target, &filters),
            vec![
                CoverageShortfall {
                    host_id: "host-elsewhere".to_string(),
                    source: OPENCODE_SOURCE.to_string(),
                    partial: false,
                },
                CoverageShortfall {
                    host_id: "host-half".to_string(),
                    source: OPENCODE_SOURCE.to_string(),
                    partial: true,
                },
            ]
    ...
    /// `shortfalls` must honour the same host/source filters `status` does, otherwise the reason
    /// shown would name pairs the user has already filtered out of the chart.
    #[test]
    fn legacy_full_bucket_has_no_shortfalls_and_filters_narrow_the_explanation() {
        let temp = tempfile::tempdir().expect("coverage tempdir");
        let mut archive = Archive::open_in_data_dir(temp.path()).expect("open archive");
        replace_origin_coverage(
            archive.connection_mut(),
        ...
        let coverage = CoverageStore::load(archive.connection()).expect("load coverage snapshot");
        let target = bucket(0, 100);
        let only_covered = AggregateFilters {
            host_id: Some("host-covered".to_string()),
            ..AggregateFilters::default()
        };

        assert_eq!(
            coverage.status(&target, &only_covered),
            CoverageStatus::Full
        );
        assert!(coverage.shortfalls(&target, &only_covered).is_empty());
        assert_eq!(
            coverage
                .shortfalls(&target, &AggregateFilters::default())
                .iter()
                .map(|shortfall| shortfall.host_id.as_str())
                .collect::<Vec<_>>(),
            vec!["host-gap"]
        );
    }

    /// A zero-length bucket is `None` by definition, and there is nothing to explain about it —
    /// emitting every pair as "missing" would flood the UI with a note the user cannot act on.
    #[test]
    fn legacy_zero_length_bucket_yields_no_shortfalls() {
        let temp = tempfile::tempdir().expect("coverage tempdir");
        let mut archive = Archive::open_in_data_dir(temp.path()).expect("open archive");
        replace_origin_coverage(
            archive.connection_mut(),
        ...
        assert_eq!(
            coverage.status(&bucket(50, 50), &filters),
            CoverageStatus::None
        );
        assert!(coverage.shortfalls(&bucket(50, 50), &filters).is_empty());
    }
