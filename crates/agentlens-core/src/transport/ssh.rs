...
//! `NumberOfPasswordPrompts=1`、进程无 TTY。口令本身经 [`AskpassChannel`] 交付：每个 ssh/scp
//! 进程各自认证，助手读一次就 unlink，所以 [`SshAuthentication::Askpass`] 持有的是
//! [`AskpassProvisioner`] 工厂而不是一个固定路径 —— 一个共享文件只够第一个进程用，后五个会
//! 拿不到口令，症状与「钥匙串里的口令不生效」完全一样。
//! Windows 下 ssh/scp 定位顺序为 PATH → 显式设置 → `%WINDIR%\System32\OpenSSH`，
...
/// Windows `CreateProcess` 标志位，以及 `run_process` 三条 spawn 路径共用的组合规则。
///
/// 之所以把它放在平台无关的位置并对 `test` 开放：Windows 专有的「有没有弹出控制台窗口」
/// 在 Linux 上无法观测，但决定它的标志位计算是纯函数，可以被跨平台测试锁死。
...
mod creation_flags {
    ...
    /// 子进程创建即挂起，等 job object attach 完成后由 `ProcessTree::attach` 唤醒。
    ...
    const _: () = {
        ...
        assert!(CREATE_NO_WINDOW == threading::CREATE_NO_WINDOW);
        assert!(DETACHED_PROCESS == threading::DETACHED_PROCESS);
        assert!(CREATE_SUSPENDED == threading::CREATE_SUSPENDED);
    };

    /// 计算 `run_process` 要传给 `CreateProcess` 的完整标志位。
    ///
    /// **为什么三条路径都必须带 `CREATE_NO_WINDOW`：** `Command::creation_flags` 是整体
    /// 赋值而非累加，重复调用会覆盖前一次，所以标志必须在一处算全。历史实现把它拆在两个
    /// helper 里 —— `ProcessTree::prepare` 管 timeout 路径、`configure_detached` 管
    /// detached 路径 —— 于是「既无 timeout 又非 detached」的第三条路径一个标志都没设。
    /// 那条路径 spawn 出的 `ssh.exe` / `scp.exe` 是 CONSOLE 子系统进程，用户在主机卡上
    /// 点「刷新」就会看到一个 cmd 黑框闪过（真实用户反馈的缺陷）。它**不是**「标志传不到
    /// 孙进程」的问题：`agentlens-askpass` 自身已经是 GUI 子系统。新增分支必须走这个函数，
    /// 不要把标志再拆回各 helper。
    pub(super) const fn windows_creation_flags(has_timeout: bool, detached: bool) -> u32 {
        if has_timeout {
...
fn run_process(
    ...
    // 三条分支之后统一赋值一次。`creation_flags` 覆盖而非累加，分散设置就会漏掉分支。
    #[cfg(windows)]
    {
        use std::os::windows::process::CommandExt as _;

        command.creation_flags(creation_flags::windows_creation_flags(
            timeout.is_some(),
            spec.detached,
        ));
    }
...
#[cfg(windows)]
impl ProcessTree {
    /// `CREATE_SUSPENDED` 不在这里设置，而是与 `CREATE_NO_WINDOW` 一起由
    /// `creation_flags::windows_creation_flags` 在 `run_process` 里一次算全。
...
/// 非 Unix 平台没有 `setsid`。Windows 的 `DETACHED_PROCESS` 也不在这里设置 —— 它与
/// `CREATE_NO_WINDOW` 一起由 `creation_flags::windows_creation_flags` 在 `run_process`
/// 里一次算全，避免又出现「某条分支没设标志」的黑框缺陷。
#[cfg(not(unix))]
...
/// Materialises one single-read askpass channel for one OpenSSH process.
///
/// It is a factory rather than a ready-made path because every `ssh`/`scp` process
/// authenticates on its own — one [`SshTransport::collect`] launches six of them — and the
/// helper unlinks the channel after its single read. A single pre-created file would
/// therefore serve the first process and leave the remaining five without a secret, which
/// looks exactly like "the stored password does not work".
///
/// The factory returns the secret **inside a channel it created**, never as bytes the
/// transport could log: everything core sees is a path.
pub type AskpassProvisioner = std::sync::Arc<dyn Fn() -> Result<AskpassChannel> + Send + Sync>;

/// A provisioned one-shot askpass channel: a private file holding exactly one secret.
///
/// # Why a `0600` file rather than an environment variable
///
/// The secret must reach a process **OpenSSH** spawns, not one we spawn, so the only channels
/// available are the filesystem and inheritance-based ones. An environment variable would put
/// the plaintext in `/proc/<pid>/environ` of the ssh process and of every child it spawns, for
/// the whole lifetime of the connection. A file created with `O_EXCL` and mode `0600` is
/// readable only by this user (and root), is not inherited by anything, and stops existing the
/// moment the helper reads it — the helper unlinks it before writing a byte to stdout.
///
/// A FIFO would keep the plaintext out of the filesystem entirely, and the helper supports one,
/// but `fs::write` to a FIFO blocks until a reader opens it and OpenSSH may never spawn the
/// helper at all (agent or key authentication succeeding first), so the writer would hang. The
/// helper's own contract accepts both; the transport deliberately uses the non-blocking half.
///
/// Residual exposure, accepted: another process running **as the same user** can read the file
/// during the milliseconds it exists — but that process can equally read the OS keyring or
/// `ptrace` this one, so same-user is outside the boundary this defends. A `SIGKILL` between
/// creation and consumption leaves one `0600` file in the temp directory until the next reboot.
#[derive(Debug)]
...
impl AskpassChannel {
    /// Write `secret` into a fresh private channel.
    ///
    /// `create_new` (`O_EXCL`) is what makes this safe against a same-user process that
    /// pre-created the path: the open fails instead of writing the secret into a file, symlink
    /// or FIFO somebody else controls. On Unix the mode is set **at creation**, so the file is
    /// never briefly world-readable; on Windows the per-user ACL of `%LOCALAPPDATA%\Temp`
    /// already excludes other users.
    pub fn create(secret: &[u8]) -> Result<Self> {
        let sequence = ASKPASS_CHANNEL_SEQUENCE.fetch_add(1, Ordering::Relaxed);
        let path = env::temp_dir().join(format!(
            ".agentlens-askpass-{}-{sequence}",
            std::process::id()
        ));
        ...
        // Only the path appears in the error — the secret is never part of a message.
        let mut file = options.open(&path).map_err(|error| SshError::AuthFailed {
            detail: format!("无法创建一次性口令通道 {}：{error}", path.display()),
        })?;
        let channel = Self { path };
        file.write_all(secret)
            .and_then(|()| file.flush())
            .map_err(|error| SshError::AuthFailed {
                detail: format!("无法写入一次性口令通道 {}：{error}", channel.path.display()),
            })?;
        Ok(channel)
    }
...
impl Drop for AskpassChannel {
    /// Removes the channel when the helper never consumed it.
    ///
    /// A missing file is the normal outcome — the helper unlinks it after its single read — so
    /// the failure is discarded rather than reported. What matters is that a connection which
    /// authenticated by key, failed before the prompt, or was cancelled leaves no plaintext
    /// behind.
    fn drop(&mut self) {
...
///
/// `Debug` is hand-written and `PartialEq` deliberately absent: the askpass variant carries a
/// closure that reaches into the OS keyring, which is neither comparable nor printable, and
/// deriving something plausible over it would invite printing the thing it fetches.
#[derive(Clone)]
...
impl fmt::Debug for SshAuthentication {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Batch { identity_file } => formatter
                .debug_struct("Batch")
            ...
            Self::Askpass {
                ...
                .field("identity_file", identity_file)
                .field("provision", &"<one-shot channel factory>")
                .finish(),
        }
    }
}

/// Authentication applied to one command: the channel lease must outlive the process.
struct AppliedAuthentication {
    detached: bool,
    /// `Some` only for askpass. Dropping it removes an unconsumed channel, so it is held until
    /// the process it authenticates has exited.
    _channel: Option<AskpassChannel>,
}
    ...
    fn apply(
        ...
        match self {
            ...
            Self::Askpass {
                ...
                let channel = provision()?;
                // One channel serves exactly one prompt, so ssh is told to ask exactly once.
                // Without this the second and third attempt find the channel already unlinked
                // and fail with a confusing "channel unavailable" instead of a plain auth
                // failure — and re-sending the same rejected secret could not have helped.
                args.push("-o".into());
                ...
                env.insert(
                    ASKPASS_CHANNEL_ENV.into(),
                    channel.path().as_os_str().to_owned(),
                );
                Ok(AppliedAuthentication {
                    detached: true,
...
pub struct SshCollectRequest {
    ...
    pub snapshot: bool,
    /// Adapter to run remotely; `None` leaves the collector on its default source.
    pub source: Option<String>,
}
...
impl<R: CommandRunner> SshTransport<R> {
    ...
    fn run_ssh_command_with_cancel(
        ...
        let applied = self.authentication.apply(&mut args, &mut command_env)?;
        args.push("--".into());
        ...
        // The channel outlives the process that had to read it; dropping it here removes the
        // file if ssh authenticated some other way and never invoked the helper.
        drop(applied);
...
AGENTLENS_MACHINE_ID_HASH=a1b2c3d4e5f60718293a4b5c6d7e8f90a1b2c3d4e5f60718293a4b5c6d7e8f90\n";
    ...
    /// 回归锁：`run_process` 的每一条 spawn 路径都必须带 `CREATE_NO_WINDOW`。
    ///
    /// Windows 上「有没有闪出 cmd 黑框」在 Linux 上无法直接观测，但决定它的标志位计算是
    /// 平台无关的纯函数，所以在任何平台上都能断言。历史缺陷正是第三条路径（既无 timeout
    /// 又非 detached）一个标志都没设，用户在主机卡上点「刷新」就看到黑框闪过。
    #[test]
    fn windows_creation_flags_set_no_window_on_every_spawn_path() {
        use super::creation_flags::{
            windows_creation_flags, CREATE_NO_WINDOW, CREATE_SUSPENDED, DETACHED_PROCESS,
        };

        // 常量本身对齐 Win32 文档值，防止手抄位移写错而测试仍然自洽。
        assert_eq!(CREATE_NO_WINDOW, 0x0800_0000);
        assert_eq!(DETACHED_PROCESS, 0x0000_0008);
        assert_eq!(CREATE_SUSPENDED, 0x0000_0004);

        let timed = windows_creation_flags(true, false);
        let timed_detached = windows_creation_flags(true, true);
        let detached = windows_creation_flags(false, true);
        let plain = windows_creation_flags(false, false);

        for (label, flags) in [
            ("timeout", timed),
            ("timeout+detached", timed_detached),
            ("detached", detached),
            ("plain", plain),
        ] {
            assert_eq!(
                ...
                CREATE_NO_WINDOW,
                "{label} 路径缺少 CREATE_NO_WINDOW：Windows 上会闪出 cmd 黑框"
            );
        }

        // 既有语义必须原样保留：timeout 路径靠 job object 收容进程树，需要先挂起再 attach。
        assert_eq!(timed & CREATE_SUSPENDED, CREATE_SUSPENDED);
        assert_eq!(timed_detached & CREATE_SUSPENDED, CREATE_SUSPENDED);
        assert_eq!(
            timed & DETACHED_PROCESS,
            0,
            "timeout 路径此前不设 DETACHED_PROCESS，本次修复不得改变它"
        );
        assert_eq!(detached & DETACHED_PROCESS, DETACHED_PROCESS);
        assert_eq!(
            detached & CREATE_SUSPENDED,
            0,
            "非 timeout 路径没有 job object 需要 resume，挂起会导致进程永不推进"
        );
        assert_eq!(
            plain, CREATE_NO_WINDOW,
            "第三条路径应恰好只带 CREATE_NO_WINDOW"
        );

        println!(
            "creation_flags timed={timed:#010x} timed_detached={timed_detached:#010x} detached={detached:#010x} plain={plain:#010x}"
        );
    }
    ...
    const ASKPASS_SECRET: &str = "fixture-login-secret";

    /// Askpass authentication whose factory records every channel it hands out, so a test can
    /// assert both the per-process count and that nothing survived the command.
    fn askpass_authentication(
        identity_file: Option<PathBuf>,
    ) -> (SshAuthentication, Arc<Mutex<Vec<PathBuf>>>) {
        let issued = Arc::new(Mutex::new(Vec::new()));
        let recorder = Arc::clone(&issued);
        let authentication = SshAuthentication::Askpass {
            executable: PathBuf::from("/opt/agentlens/agentlens-askpass"),
            provision: Arc::new(move || {
                let channel = AskpassChannel::create(ASKPASS_SECRET.as_bytes())?;
                recorder
                    .lock()
                    .expect("issued channel lock")
                    .push(channel.path().to_path_buf());
                Ok(channel)
            }),
            identity_file,
        };
    ...
    fn ssh_askpass_sets_force_environment_detaches_and_omits_batch_mode() {
        ...
        runner.push(CommandStage::Stage1, 0, PROBE_X86_64, "");
        let (_temp, artifacts) = artifact();
        let (authentication, issued) =
            askpass_authentication(Some(PathBuf::from("/keys/id_ed25519")));
        let transport = SshTransport::new(runner.clone(), tools(), authentication, artifacts)
        ...
        let channels = issued.lock().expect("issued channel lock").clone();
        assert_eq!(
            command
                .env
                ...
                .map(PathBuf::from)
                .as_deref(),
            channels.first().map(PathBuf::as_path),
            "the helper must be pointed at the channel this command provisioned"
        );
        ...
        assert!(
            ...
            "a single-read channel must be paired with a single prompt"
        );
    }

    /// Every ssh/scp process authenticates on its own and the helper consumes the channel it
    /// read, so a shared channel would starve every process after the first. This pins the
    /// one-channel-per-process contract and the "no plaintext survives" cleanup.
    #[test]
    fn ssh_askpass_provisions_one_channel_per_process_and_leaves_none_behind() {
        let runner = FakeRunner::default();
        push_startup(&runner);
        push_successful_collection(&runner);
        let (_temp, artifacts) = artifact();
        let (authentication, issued) = askpass_authentication(None);
        let transport = SshTransport::new(runner.clone(), tools(), authentication, artifacts)
            .expect("construct askpass transport");

        transport
            .collect(&request())
            .expect("askpass collection succeeds against the fake runner");

        let channels = issued.lock().expect("issued channel lock").clone();
        let authenticated = runner
            .commands()
            .into_iter()
            .filter(|command| command.stage != CommandStage::StartupProbe)
            .count();
        assert_eq!(
            channels.len(),
            authenticated,
            "each authenticating process needs its own one-shot channel"
        );
        let unique: std::collections::BTreeSet<_> = channels.iter().collect();
        assert_eq!(
            unique.len(),
            channels.len(),
            "channels must never be reused"
        );
        for channel in &channels {
            assert!(
                !channel.exists(),
                "channel {} outlived the process it authenticated",
                channel.display()
            );
        }
    }
    ...
    #[test]
    fn ssh_askpass_channel_holds_the_secret_privately_and_vanishes_on_drop() {
        let channel = AskpassChannel::create(ASKPASS_SECRET.as_bytes()).expect("create channel");
        let path = channel.path().to_path_buf();
        assert_eq!(
            fs::read(&path).expect("read channel"),
            ASKPASS_SECRET.as_bytes(),
            "the helper reads the secret verbatim, with no trailing newline added here"
        );
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt as _;
            let mode = fs::metadata(&path).expect("channel metadata").permissions();
            assert_eq!(
                mode.mode() & 0o777,
                0o600,
                "the channel must never be readable by another user"
            );
        }

        drop(channel);
        assert!(!path.exists(), "dropping the lease must remove the secret");

        let stale = AskpassChannel::create(ASKPASS_SECRET.as_bytes()).expect("create channel");
        let path = stale.path().to_path_buf();
        fs::remove_file(&path).expect("simulate the helper consuming the channel");
        drop(stale);
        assert!(!path.exists());
    }

    /// A failing factory must abort the command rather than silently fall back to a prompt-less
    /// connection, and the failure text must not carry the secret it could not deliver.
    #[test]
    fn ssh_askpass_provisioning_failure_fails_authentication_without_leaking() {
        let runner = FakeRunner::default();
        push_startup(&runner);
        let (_temp, artifacts) = artifact();
        ...
        .expect("startup probe needs no channel");

        let error = transport
            .collect(&request())
            .expect_err("an unusable channel must fail the round");
        assert!(matches!(error, SshError::AuthFailed { .. }));
        assert!(!error.to_string().contains(ASKPASS_SECRET));
        assert!(!format!("{error:?}").contains(ASKPASS_SECRET));
        assert!(runner
            .commands()
            .iter()
            .all(|command| command.stage == CommandStage::StartupProbe));
    }

    #[test]
    fn ssh_authentication_debug_redacts_the_channel_factory() {
        let (authentication, _issued) = askpass_authentication(Some(PathBuf::from("/keys/id")));
        let rendered = format!("{authentication:?}");
        assert!(rendered.contains("agentlens-askpass"));
        assert!(rendered.contains("/keys/id"));
        assert!(rendered.contains("<one-shot channel factory>"));
        assert!(!rendered.contains(ASKPASS_SECRET));
        assert!(format!(
            "{:?}",
            SshAuthentication::Batch {
                identity_file: None
