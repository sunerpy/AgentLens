...
> Both URLs above name the real repository, sunerpy/AgentLens, but that
> repository has not been created yet, so both 404 today. The scripts have been
> verified against a local source only; they have never fetched a real release.
> See [repo-metadata.md](repo-metadata.md) for the full list of what that blocks.
