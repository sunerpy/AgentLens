...
[`Codex`](adapters/codex.md) and [`Hermes`](adapters/hermes.md) are implemented,
local and remote alike; a single host can have several enabled.
`SUPPORTED_SOURCES` now holds those four. Claude Code, Codex and Hermes have all
been reconciled bucket by bucket against real data, but the repository carries no
real fixture, so every automated test still runs on synthetic data.
`hosts.enabled_sources` defaults to `'opencode'`, so the other three have to be
enabled explicitly. Codex deliberately does not implement `.jsonl.zst`
decompression and skips such files whole, counting them.

## Record granularity

The first three sources are per-message; Hermes is per-session. Hermes leaves
`messages.token_count` NULL throughout, and the five buckets only exist on the
`sessions` table, so the session totals cannot be rebuilt from messages. That is
a property of the source, not a defect. The archive therefore carries
`usage_record.granularity`, either `'message'` or `'session'`: Hermes normalises
each session into one session-level record whose `message_id` equals its
`session_id`, letting the existing `UNIQUE(host_id, source, message_id)` upsert
overwrite the running totals.

Aggregation splits along the same line. `message_count` counts only per-message
records, `session_record_count` counts only per-session records, while the five
token buckets, cost and `active_session_count`
(`count(DISTINCT session_id)`) sum across both granularities. So Hermes usage
lands in tokens and cost but never in the message count.

`granularity` is declared in the `migration_v1` baseline schema with
`DEFAULT 'message'`, and `LATEST_SCHEMA_VERSION` is still 3: the project is not
in production, so a v4 migration was deliberately skipped.

## Schema fingerprint and rebuild

Because the baseline schema is edited in place instead of gaining a migration,
an archive created by an older build can carry columns the current code cannot
query. Opening an archive therefore checks the full table-column fingerprint
against `EXPECTED_SCHEMA_COLUMNS`. On a mismatch the archive is not opened
read-only and not silently patched: a `VACUUM INTO` backup is written next to it
as `archive.db.backup-<timestamp>.db`, then the database is rebuilt on the
current baseline. Nothing has to be deleted by hand. If the fingerprint still
disagrees after a rebuild, that is a code defect — the migrations and
`EXPECTED_SCHEMA_COLUMNS` are out of sync — and it surfaces as an error rather
than as wrong query results.

## Pricing resolution

Three of the four sources carry no money, so cost is estimated at query time from
the pricing catalog. Matching is deliberately looser than exact string equality,
in two steps:

- **Cross-provider fallback.** The same model reached through different gateways
  usually has a price entry only under its owning provider, so a lookup that
  misses on `(provider, model)` may fall back to another provider's entry for the
  same model, preferring `anthropic` / `google` / `openai`, then
  `amazon-bedrock`, then the rest.
- **Variant-suffix stripping.** Eight reasoning-effort suffixes are stripped:
  `xhigh`, `high`, `medium`, `low`, `minimal`, `max`, `thinking`, `fast`. These
  are confirmed to share a price entry with the base model.

`mini` and `nano` are **not** stripped, on purpose: they are separate models with
separate prices (GPT-5.4 measures 2.5/15, 0.75/4.5 and 0.2/1.25 USD per Mtok), so
stripping them would produce an error of more than 3x. `preview` / `latest` are
release channels or rolling aliases and `free` is a billing tier, so none of them
can be assumed equivalent either.

Measured over 251737 archived records, the two steps took the priceable share
from 0.1% to 99.4%. Manual overrides do not participate in any of this: a
user-supplied price matches `(provider, model)` exactly, so an override can never
bleed onto another provider's records. The price editor can pre-fill from an
existing catalog entry, which is a convenience in the UI, not a change to the
matching rule. The archive's model key includes a `variant`, but price entries
deliberately ignore it: prices vary by model, not by reasoning effort.

## Aggregation performance

The trend query is a single SQL statement rather than one query per bucket. On a
251737-record archive that took the measured wall time from 6123 ms to 514 ms, a
12x improvement. `migration_v1` carries four composite indexes for it, on
`(source, time_created_utc)`, `(agent_key, time_created_utc)`,
`(provider_id, time_created_utc)` and `(model_id, time_created_utc)`. They cost
22.7 MB of database growth, +41.55%, which is the accepted trade for the latency.
...
- The window drop shadow and the Windows 11 rounded corners. Both depend on
  `shadow: true`, so it is declared explicitly: `shadow: false` would drop the
  corners along with the shadow.
