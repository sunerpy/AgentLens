...
本契约已落地：解析代码在 `crates/agentlens-core/src/source/claude_code.rs`，远端路径由采集器
`agentlens-collector --source claude-code` 驱动，本地路径由 `src-tauri/src/state.rs` 的
`ClaudeCodeLocalSource::discover` 驱动，调度器以 `(host_id, source)` 为键，同一主机可与
OpenCode 并行采集。

已对真实数据实测对账：本机 `~/.claude/projects` 的 645 个 jsonl / 5222 行跑过全量采集，
17 条 `messageId` 全部命中、零缺失，五桶 `in=34925 out=7123 cr=72196 cw=290010`
合计 404254 与独立提取逐桶一致。`eligible_count=17 / skipped_count=5205` 不是丢数据：
5222 行里只有 21 行带 `message.usage`，21→17 是同一 `messageId` 的重复行被正确去重。

两条现实限制：这轮对账跑在本机私人数据上，数据不入库，所以仓库内的 14 个 claude 测试
仍全部基于合成清单，没有真实 `~/.claude/projects` fixture；`hosts.enabled_sources`
的列默认值是 `'opencode'`，既有主机升级后不会自动采集 Claude Code，需要显式启用。
