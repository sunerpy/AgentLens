...
本契约已落地：解析代码在 `crates/agentlens-core/src/source/codex.rs`，远端路径由采集器
`agentlens-collector --source codex` 驱动，`SUPPORTED_SOURCES` 已含 `codex`，调度器以
`(host_id, source)` 为键，同一主机可与 OpenCode、Claude Code 并行采集。

已对真实数据实测对账：`~/.codex` 的 220 个 rollout（`sessions/` 215 +
`archived_sessions/` 5）采出 `eligible_count=20252 / skipped_count=132702`，五桶
`in=2494715106 out=10995018 rz=5511044 cr=2151336078 cw=49821053` 与独立提取逐个一致，
零缺失零多余，定价命中 19952/20252。该对账跑在本机私人数据上，数据不入库，所以仓库内的
5 个 `#[test]` 仍基于合成 fixture。

两条现实限制：`.jsonl.zst` **有意不实现**。契约要求流式解压，但本机零个 zst 文件且
workspace 无 zstd 依赖，本轮遇到即整文件跳过并计数，这是范围限制，不是缺陷。另外
`hosts.enabled_sources` 的列默认值是 `'opencode'`，既有主机升级后不会自动采集 Codex，
需要显式启用。
...
| `provider_id` | 最近 `turn_context.model` 的合法 namespace；否则回退 `session_meta.model_provider` | `openai.gpt-5.4` → `openai`；两者都缺失时写 `"codex"` |
| `model_id` | 最近 `turn_context.model` 去除合法 provider namespace 后的部分 | `openai.gpt-5.4` → `gpt-5.4`；无 namespace 时保留原值；**不取自 `token_count` 事件** |
...
> **provider 归一化是有意偏离原始预留契约。** `session_meta.model_provider` 描述的是接入通道，
> 例如通过 `amazon-bedrock` 转发 OpenAI 模型；`turn_context.model` 的 `openai.` namespace 才是
> 可定价模型所属的 provider。价格匹配的 exact、normalized、family 三层都要求 provider 相等，
> 而价目目录中的 GPT-5.4 / GPT-5.6 只登记在 `openai` 下。若直接保留
> `amazon-bedrock`，实测 17,317 / 20,252（约 85%）条记录无法估价。没有合法 namespace 的
> `codex-auto-review` 等标识仍回退 `session_meta.model_provider`，不会被误拆或误定价。
