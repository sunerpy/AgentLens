...
export function getTrend(
  ...
  return invoke<SeriesQueryResult>('get_trend', { range, tz, granularity, filters })
}
...
/**
 * The source keys a host may be configured to collect, exported from
 * `agentlens_core::host::SUPPORTED_SOURCES`. Read it instead of writing the list here: a
 * TypeScript copy drifts the moment a fifth adapter lands, and the drift surfaces only as a
 * `hosts_update` the backend rejects.
 */
export function hostsSupportedSources(): Promise<string[]> {
  return invoke<string[]>('hosts_supported_sources')
}
...
/**
 * Runs one refresh round per **enabled source** on the host, so the result is a list: the
 * scheduler is keyed `(host_id, source)` and each slot reports its own outcome.
 */
