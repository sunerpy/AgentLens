...
function point(
  ...
  sessionRecordCount: number | null = null,
): SeriesPoint {
  return {
...
describe('trendModel/coverage 三态：空洞 ≠ 零', () => {
  ...
  it('会话汇总记录数原样透传，含 null 与 0 两种非等价情形', () => {
    const rows = toTrendRows([
      point('01-01', 'none', null, null, null, null),
      point('01-02', 'full', TOKENS_SAMPLE, COST_SAMPLE, 12, 0),
      point('01-03', 'full', TOKENS_SAMPLE, COST_SAMPLE, 0, 7),
    ])
    expect(rows.map((row) => row.sessionRecordCount)).toEqual([null, 0, 7])
    // 会话级记录不参与消息数：第三桶消息数为 0 而会话汇总记录数为 7。
    expect(rows[2].messageCount).toBe(0)
  })
