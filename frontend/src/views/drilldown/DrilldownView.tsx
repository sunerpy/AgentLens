...
function StatChip({ label, value, testId }: { label: string; value: string; testId?: string }) {
  return (
    <span
      data-testid={testId}
      className="flex flex-col gap-0.5 rounded-lg border border-border bg-card px-3 py-1.5"
    >
...
export function DrilldownView() {
  ...
  /**
   * Derived from the whole unfiltered result, not from the visible level: drilling into an
   * OpenCode source would otherwise drop the column mid-navigation and make the three levels
   * disagree about their column contract.
   */
  const showSessionRecords = useMemo(() => rows.some((row) => row.sessionRecordCount > 0), [rows])
    ...
    <section data-testid="view-drilldown" className="flex flex-col gap-5">
      ...
      {breakdownQuery.isSuccess && rows.length > 0 ? (
        <>
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              ...
              {showSessionRecords ? (
                <StatChip
                  ...
                  value={formatCount(filteredTotal.sessionRecordCount)}
                  testId="drilldown-total-session-records"
                />
              ) : null}
            </div>
          </div>
          ...
          {showSessionRecords ? (
            <p
              data-testid="drilldown-session-record-note"
              className="rounded-lg border border-dashed border-border bg-muted/30 px-3 py-2 text-xs leading-relaxed text-muted-foreground"
            >
              {zh.drilldown.sessionRecordNote}
            </p>
          ) : null}

          <SourceLevel
            nodes={sources}
            showSessionRecords={showSessionRecords}
