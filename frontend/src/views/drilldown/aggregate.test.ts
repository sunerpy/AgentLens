...
describe('aggregate/按来源分组保留粒度差异', () => {
  it('只提供会话级计量的来源：消息数 0 而会话汇总记录数与 token 都有值', () => {
    const nodes = groupBySource([
      row({ source: 'opencode', tokens: tokens({ tokInput: 100 }), messageCount: 9 }),
      row({
        source: 'hermes',
        tokens: tokens({ tokInput: 400 }),
        messageCount: 0,
        sessionRecordCount: 7,
      }),
    ])
    const hermes = nodes.find((node) => node.source === 'hermes')
    expect(hermes?.metric.messageCount).toBe(0)
    expect(hermes?.metric.sessionRecordCount).toBe(7)
    expect(hermes?.metric.tokens.tokInput).toBe(400)

    const opencode = nodes.find((node) => node.source === 'opencode')
    expect(opencode?.metric.sessionRecordCount).toBe(0)
  })
})
...
describe('aggregate/sumMetrics', () => {
  ...
  it('会话汇总记录数独立累加，不与消息数互相污染', () => {
    const total = sumMetrics([
      row({ messageCount: 4, sessionRecordCount: 0 }),
      row({ source: 'hermes', messageCount: 0, sessionRecordCount: 7 }),
    ])
    expect(total.messageCount).toBe(4)
    expect(total.sessionRecordCount).toBe(7)
    expect(total.rowCount).toBe(2)
  })

  it('emptyMetric 的会话汇总记录数从 0 起算', () => {
    expect(emptyMetric().sessionRecordCount).toBe(0)
  })
