...
export function usePriceOverrides() {
  ...
  /**
   * `rates` carries what the caller already typed, which the inline observed-model form supplies.
   * Without it the row would land pre-filled from `matchedPrice` and discard the edit the user
   * just made in the expanded panel.
   */
  const addDraft = (
    entry?: ObservedModelPrice,
    rates?: Partial<Record<PriceNumericField, string>>,
  ) => {
    ...
    mutate([
      ...rows,
      {
        ...
        providerId: entry?.providerId ?? '',
        modelId: entry?.modelId ?? '',
        inputPerMtok: rates?.inputPerMtok ?? String(matched?.inputPerMtok ?? 0),
        outputPerMtok: rates?.outputPerMtok ?? String(matched?.outputPerMtok ?? 0),
        cacheReadPerMtok: rates?.cacheReadPerMtok ?? String(matched?.cacheReadPerMtok ?? 0),
        cacheWritePerMtok: rates?.cacheWritePerMtok ?? String(matched?.cacheWritePerMtok ?? 0),
        extra: {},
      },
    ])
