...
/**
 * 共享报表区间的 reducer。三个视图（总览 / 用量分析 / 明细）都从它取"我在看哪个窗口"，
...
describe('reportRange/defaultGranularity', () => {
  ...
  it('31 天仍是天桶，32 天起转周桶', () => {
    expect(defaultGranularity(range('2024-01-01', '2024-02-01'))).toBe('day')
    expect(defaultGranularity(range('2024-01-01', '2024-02-02'))).toBe('week')
  })

  it('季度跨度落周桶：92 天是周桶上界，93 天起转月桶', () => {
    expect(defaultGranularity(range('2024-10-01', '2025-01-01'))).toBe('week')
    expect(defaultGranularity(range('2024-09-30', '2025-01-01'))).toBe('month')
  })

  it('整年落月桶——Granularity 没有 year 变体，年窗口画成 12 个月桶', () => {
    expect(defaultGranularity(range('2024-01-01', '2025-01-01'))).toBe('month')
    expect(defaultGranularity(range('2023-01-01', '2024-01-01'))).toBe('month')
  })
})
