...
test('next page issues a new offset=50 call and renders different rows', async ({ page }) => {
  ...
  await expect(
    page.locator('[data-testid="detail-row"][data-message-id="msg_mock_0050"]'),
  ).toBeVisible()
  await expect(
    page.locator('[data-testid="detail-row"][data-message-id="msg_mock_0099"]'),
  ).toBeVisible()
  await expect(
    page.locator('[data-testid="detail-row"][data-message-id="msg_mock_0000"]'),
  ).toHaveCount(0)
  await expect(page.getByTestId('detail-row')).toHaveCount(PAGE_SIZE)
...
test('the last page is bounded by total_count', async ({ page }) => {
  ...
  await expect.poll(async () => (await lastQueryArgs(page)).offset).toBe(2 * PAGE_SIZE)
