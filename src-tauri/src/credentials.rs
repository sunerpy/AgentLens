...
//!
//! # How a stored secret reaches `ssh`
//!
//! [`ssh_authentication_for_host`] is the bridge: it picks the entry `ssh` will be prompted
//! for and returns an [`SshAuthentication::Askpass`] whose provisioner re-reads the keyring
//! once per `ssh`/`scp` process, writing the plaintext into a private single-read channel that
//! the bundled helper consumes and unlinks. When no secret is stored, or the helper is missing,
//! the caller keeps `BatchMode` — an immediate, legible authentication failure instead of a
//! hang. Nothing here holds plaintext between connections, and [`secret_is_deliverable`] is
//! what stops `present` from claiming a secret is in effect when it provably cannot be.
//!
...
use agentlens_core::transport::ssh::{
    AskpassChannel, RemoteArchitecture, SshAuthentication, SshError, SshProbe,
};
...
// ---------------------------------------------------------------------------
// Delivering a stored secret to OpenSSH
// ---------------------------------------------------------------------------

/// Override for the bundled `SSH_ASKPASS` helper. Development builds and tests use it.
pub const ASKPASS_ENV: &str = "AGENTLENS_ASKPASS";

#[cfg(windows)]
const ASKPASS_BINARY: &str = "agentlens-askpass.exe";
#[cfg(not(windows))]
const ASKPASS_BINARY: &str = "agentlens-askpass";

/// The bundled askpass helper, or `None` when this installation has none.
///
/// Resolution mirrors `crate::state::collector_artifacts`: an explicit environment override
/// first, then the sibling of the running executable — which is where every bundler puts it
/// (`/usr/bin/agentlens-askpass` beside `/usr/bin/AgentLens`, `agentlens-askpass.exe` beside
/// `AgentLens.exe`, `target/debug/agentlens-askpass` beside the dev binary).
///
/// Without the helper a stored secret cannot reach `ssh` at all, so its absence is what makes a
/// stored credential undeliverable — the fact `credential_status` reports.
pub fn askpass_helper_path() -> Option<std::path::PathBuf> {
    std::env::var_os(ASKPASS_ENV)
        .map(std::path::PathBuf::from)
        .filter(|path| path.is_file())
        .or_else(|| {
            let path = std::env::current_exe().ok()?.parent()?.join(ASKPASS_BINARY);
            path.is_file().then_some(path)
        })
}

/// Which secret `ssh` will be asked for, or `None` when the host has none stored.
///
/// With an identity file configured the first prompt is the key's passphrase, so that entry
/// wins; otherwise `ssh` falls through to password authentication and the login password wins.
/// The other kind is the fallback in both directions, which keeps a host that stored only one
/// of the two working either way.
///
/// Only one kind can be served per connection: the helper receives OpenSSH's prompt text but
/// ignores it by contract, so it cannot pick between two secrets. A read failure is treated as
/// absence — degrading to `BatchMode` produces a clear authentication error, whereas guessing
/// would produce a hang.
fn stored_credential_kind(
    store: &dyn CredentialStore,
    host_id: &str,
    has_identity_file: bool,
) -> Option<CredentialKind> {
    let order = if has_identity_file {
        [CredentialKind::Passphrase, CredentialKind::Password]
    } else {
        [CredentialKind::Password, CredentialKind::Passphrase]
    };
    order.into_iter().find(|kind| {
        let reference = CredentialRef::new(host_id, *kind);
        match store.status(&reference) {
            Ok(status) => status.present,
            Err(error) => {
                // `CredentialError` carries the account name and the backend's own text,
                // never the secret, so this is safe to log verbatim.
                tracing::warn!(
                    host_id,
                    kind = kind.as_str(),
                    %error,
                    "unable to read the keyring; treating the credential as absent"
                );
                false
            }
        }
...
/// SSH authentication for one host: askpass when a usable secret exists, batch otherwise.
///
/// The secret is deliberately **not** captured here. The returned provisioner re-reads the
/// keyring once per `ssh`/`scp` process, so this process never holds plaintext between
/// connections and a secret deleted mid-round stops being used immediately.
///
/// `helper` comes from [`askpass_helper_path`] in production and is a parameter so both
/// branches are reachable in a test binary, whose executable has no bundled sibling.
pub fn ssh_authentication_for_host(
    store: Arc<dyn CredentialStore>,
    host_id: &str,
    identity_file: Option<std::path::PathBuf>,
    helper: Option<std::path::PathBuf>,
) -> SshAuthentication {
    let Some(executable) = helper else {
        return SshAuthentication::Batch { identity_file };
    };
    let Some(kind) = stored_credential_kind(store.as_ref(), host_id, identity_file.is_some())
    else {
        return SshAuthentication::Batch { identity_file };
    };
    let reference = CredentialRef::new(host_id, kind);
    tracing::debug!(
        host_id,
        kind = kind.as_str(),
        "handing the stored credential to ssh through the askpass helper"
    );
    SshAuthentication::Askpass {
        executable,
        provision: Arc::new(move || provision_askpass_channel(store.as_ref(), &reference)),
        identity_file,
    }
}

/// Read one secret out of the keyring and into a fresh one-shot channel.
///
/// The plaintext exists as a [`Secret`] for the two statements it takes to write the channel,
/// and only [`Secret::expose`] can produce it. Failure text carries the keyring's own error,
/// which by construction never contains the secret.
fn provision_askpass_channel(
    store: &dyn CredentialStore,
    reference: &CredentialRef,
) -> std::result::Result<AskpassChannel, SshError> {
    match store.read(reference) {
        Ok(CredentialLookup::Present(secret)) => AskpassChannel::create(secret.expose().as_bytes()),
        Ok(CredentialLookup::Absent) => Err(SshError::AuthFailed {
            detail: format!("钥匙串条目 {} 在连接期间消失了", reference.account()),
        }),
        Err(error) => Err(SshError::AuthFailed {
            detail: format!("无法从钥匙串读取 {}：{error}", reference.account()),
        }),
    }
}
...
mod tests {
    ...
    /// Serialises the one test that mutates [`ASKPASS_ENV`]; every other test injects the helper
    /// path as a parameter precisely so it never has to touch process-wide state.
    static ASKPASS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
    ...
    impl AskpassEnvGuard {
        fn set(value: impl AsRef<std::ffi::OsStr>) -> Self {
            let previous = std::env::var_os(ASKPASS_ENV);
            std::env::set_var(ASKPASS_ENV, value);
            Self { previous }
        }
    }
    ...
    impl Drop for AskpassEnvGuard {
        fn drop(&mut self) {
            match &self.previous {
                Some(value) => std::env::set_var(ASKPASS_ENV, value),
                None => std::env::remove_var(ASKPASS_ENV),
            }
        }
    }
    ...
    fn credentials_secret_is_absent_from_every_serialized_dto() {
        ...
        let host = Host {
            enabled_sources: vec!["opencode".to_owned()],
            host_id: HOST.to_owned(),
    ...
    fn helper() -> std::path::PathBuf {
        std::path::PathBuf::from("/usr/bin/agentlens-askpass")
    }
    ...
    fn authentication_for(
        ...
        ssh_authentication_for_host(store, HOST, identity_file, Some(helper()))
    }

    /// The defect this whole module existed to avoid and still had: a secret sitting in the
    /// keyring while every SSH command was built with `BatchMode=yes`, which forbids the very
    /// prompt the secret would answer. A stored password must produce askpass authentication.
    #[test]
    fn credentials_a_stored_password_produces_askpass_authentication() {
        let store = InMemoryCredentialStore::new();
        store
            .store(&password(), &Secret::new(SECRET))
            .expect("store password");

        let authentication = authentication_for(Arc::new(store), None);

        let SshAuthentication::Askpass {
            executable,
            provision,
            identity_file,
        } = authentication
        else {
            panic!("a stored password must not fall back to BatchMode");
        };
        assert_eq!(executable, helper());
        assert_eq!(identity_file, None);

        let channel = provision().expect("provision a channel from the keyring");
        assert_eq!(
            std::fs::read(channel.path()).expect("read provisioned channel"),
            SECRET.as_bytes(),
            "the helper must find the exact stored secret in its channel"
        );
    ...
    #[test]
    fn credentials_without_a_stored_secret_or_helper_authentication_stays_batch() {
        let empty = InMemoryCredentialStore::new();
        let identity = Some(std::path::PathBuf::from("/keys/id_ed25519"));
        assert!(
            matches!(
                authentication_for(Arc::new(empty), identity.clone()),
                SshAuthentication::Batch { identity_file } if identity_file == identity
            ),
            "with nothing stored, BatchMode keeps the failure immediate and legible"
        );
        ...
        assert!(
            matches!(
                ssh_authentication_for_host(Arc::new(stored), HOST, None, None),
                SshAuthentication::Batch { .. }
            ),
            "an installation without the bundled helper cannot deliver a secret at all"
        );

        let broken = InMemoryCredentialStore::failing("session bus has no secret service");
        assert!(
            matches!(
                authentication_for(Arc::new(broken), None),
                SshAuthentication::Batch { .. }
            ),
            "an unreadable keyring must degrade to BatchMode rather than hang on a prompt"
        );
    }

    /// With an identity file, OpenSSH prompts for the key passphrase before it ever offers
    /// password authentication, so the passphrase is the entry that can actually be answered.
    /// Without one the order reverses. Getting this backwards hands `ssh` the wrong secret and
    /// looks exactly like a wrong password.
    #[test]
    fn credentials_identity_file_decides_which_of_two_stored_secrets_is_offered() {
        let store = Arc::new(InMemoryCredentialStore::new());
        store
            .store(&password(), &Secret::new("login-password"))
            .expect("store password");
        store
            .store(&passphrase(), &Secret::new("key-passphrase"))
            .expect("store passphrase");

        let offered = |identity_file: Option<std::path::PathBuf>| {
            let SshAuthentication::Askpass { provision, .. } =
                authentication_for(store.clone(), identity_file)
            else {
                panic!("two stored secrets must still produce askpass authentication");
            };
            let channel = provision().expect("provision channel");
            String::from_utf8(std::fs::read(channel.path()).expect("read channel"))
                .expect("channel holds UTF-8")
        };

        assert_eq!(
            offered(Some(std::path::PathBuf::from("/keys/id_ed25519"))),
            "key-passphrase"
        );
        assert_eq!(offered(None), "login-password");
    }

    /// The provisioner re-reads the keyring per process rather than capturing the plaintext, so
    /// a secret deleted mid-round stops being offered instead of being replayed from memory.
    #[test]
    fn credentials_provisioner_rereads_the_keyring_and_reports_a_vanished_entry() {
        let store = Arc::new(InMemoryCredentialStore::new());
        store
            .store(&password(), &Secret::new(SECRET))
            .expect("store password");
        let SshAuthentication::Askpass { provision, .. } = authentication_for(store.clone(), None)
        else {
            panic!("a stored password must produce askpass authentication");
        };

        store
            .store(&password(), &Secret::new("rotated-secret"))
            .expect("rotate password");
        let channel = provision().expect("provision after rotation");
        assert_eq!(
            std::fs::read(channel.path()).expect("read channel"),
            b"rotated-secret",
            "each process must pick up the current secret, not a captured copy"
        );
        drop(channel);

        store.delete(&password()).expect("delete password");
        let error = provision().expect_err("a vanished entry must fail authentication");
        assert!(matches!(error, SshError::AuthFailed { .. }));
        assert!(error.to_string().contains(&password().account()));
    }

    /// The one leak that would be permanent: `crate::logging` writes records to a file on disk
    /// and the feedback entry point walks operators into pasting them into a public issue.
    #[test]
    fn credentials_no_failure_path_puts_the_secret_in_an_error_message() {
        let unavailable = provision_askpass_channel(
            &InMemoryCredentialStore::failing("no secret service"),
            &password(),
        )
        .expect_err("a failing keyring must be reported");
        let vanished = provision_askpass_channel(&InMemoryCredentialStore::new(), &password())
            .expect_err("an absent entry must be reported");
        for error in [&unavailable, &vanished] {
            assert!(
                !error.to_string().contains(SECRET),
                "failure text leaked the secret: {error}"
            );
            assert!(!format!("{error:?}").contains(SECRET));
            assert!(!error.remediation().contains(SECRET));
            assert!(
                error.to_string().contains(&password().account()),
                "the account name is what makes a failure diagnosable: {error}"
            );
        }
        ...
        let authentication = authentication_for(Arc::new(store), None);
        assert!(
            !format!("{authentication:?}").contains(SECRET),
            "SshAuthentication Debug leaked the secret it can fetch"
        );
    }

    /// Executable form of the log-safety claim, on the two paths that emit records while a
    /// secret is in play. Logs are written to a file the feedback flow invites operators to
    /// paste into a public issue, so a leak there is disclosure, not a warning.
    #[test]
    fn credentials_the_authentication_bridge_writes_no_secret_to_the_tracing_log() {
        ...
        store
            ...
            .expect("store password");
        let authentication = authentication_for(Arc::new(store), None);
        let SshAuthentication::Askpass { provision, .. } = &authentication else {
            panic!("a stored password must produce askpass authentication");
        };
        drop(provision().expect("provision a channel"));

        // The other emitting path: an unreadable keyring logs the backend's own error text.
        let _batch = authentication_for(
            Arc::new(InMemoryCredentialStore::failing("no secret service")),
            None,
        );

        let logged = String::from_utf8(buffer.0.lock().expect("log buffer lock").clone())
            .expect("log records are UTF-8");
        assert!(
            !logged.is_empty(),
            "the bridge must stay diagnosable, or this test proves nothing"
        );
        assert!(
            !logged.contains(SECRET),
            "a tracing record leaked the secret: {logged}"
        );
        assert!(
            logged.contains(HOST),
            "records must still name the host they concern: {logged}"
        );
    }

    #[test]
    fn credentials_askpass_helper_is_taken_from_the_override_then_the_bundled_sibling() {
        let _environment = ASKPASS_ENV_LOCK.lock().expect("serialize env mutation");
        let directory = tempfile::tempdir().expect("create helper directory");
        let bundled = directory.path().join(ASKPASS_BINARY);

        let _missing = AskpassEnvGuard::set(directory.path().join("absent"));
        let sibling = askpass_helper_path();
        assert_ne!(
            sibling.as_deref(),
            Some(bundled.as_path()),
            "a non-existent override must never be reported as the helper"
        );

        std::fs::write(&bundled, b"fixture helper").expect("write helper fixture");
        let _override_guard = AskpassEnvGuard::set(&bundled);
        assert_eq!(askpass_helper_path().as_deref(), Some(bundled.as_path()));
    }
