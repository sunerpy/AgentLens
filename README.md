...
总览页：左侧三态侧栏，右侧是区间与粒度、四类 Token 分桶、成本卡与用量趋势图。成本按
「实际 / 估算 / 缺失」分层且永不相加，每层各自标出覆盖比例与每百万 Token 单价——单价是
这张卡里唯一可以横向比较的数。趋势图的斜纹区是无数据覆盖的断裂桶，底色区是部分覆盖，
二者都不会被当成 0，底部还逐条列出是哪个「主机 / 源」缺了采集区间。
